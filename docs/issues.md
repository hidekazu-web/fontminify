...
| ID | 課題 | カテゴリ | 状態 | 担当 |
|----|------|----------|------|------|
| ISS-018 | Web版UIでスクロールできず下部が見えない | UI/Web版 | オープン | - |
...
### ISS-018: Web版UIでスクロールできず下部が見えない
- **優先度**: P1 (中)
- **カテゴリ**: UI/Web版
- **説明**: Web版のUIでページ全体のスクロールができず、画面下部のコンテンツ（処理ボタン、フッターなど）が見えなくなる
- **再現手順**:
  1. `npm run dev:web` でWeb版を起動
  2. フォントファイルをアップロード
  3. フォント情報・文字セット選択などが表示される
  4. 画面下部にスクロールしようとしても動かない
- **影響範囲**: Web版全体のユーザビリティ
- **対応方針**: App.tsx または index.html の CSS を修正し、ページ全体のスクロールを有効化

### ISS-014: 開発環境でElectronエラー
- **優先度**: P1 (中)
- **カテゴリ**: ビルド
- **説明**: `npm run dev`実行時に`Cannot read properties of undefined (reading 'whenReady')`エラー
- **影響範囲**: 開発効率低下（DMGビルド後のアプリは正常動作）
- **確認済み事項**:
  - Electron 37.2.6 は正しくインストールされている
  - コードビルド自体は成功
  - バリアブルフォント対応以前のコミットでも同じエラー発生
- **考えられる原因**: Node.js v22.17.1 と Electron 37 の互換性問題
- **対応方針**: Node.js LTS (v20) へのダウングレードまたは node_modules 再インストール
...
| ISS-016 | カスタムタブ選択時にReactエラー | 2025-12-12 | useRefでコールバック保持して無限ループ防止 |
| ISS-017 | 出力形式選択時のレイアウト崩れ | 2025-12-12 | sr-onlyの親にrelative追加、フレックスにmin-h-0追加 |
